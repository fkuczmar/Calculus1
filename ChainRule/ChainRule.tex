...
\begin{enumerate}
\item $\frac{d}{dx} \left(  \frac{12}{1+x^2} \right)$

\item $\frac{d}{dt} \left( 12 (3)^{t/5}   \right)$

\item $\frac{d}{dt} \left(  e^{3 \sin (4t)} \right)$

\item $\frac{d}{d\theta} \left( 12 + 5 \cos (\theta/4)   \right)$
\end{enumerate}
\end{question}
...
Use the chain rule and the facts that 
\[
...
\]
to compute each of the following derivatives. \emph{Show all steps in using the chain rule as in the Explanation to Examples 2 and 9 above.}

\begin{enumerate}

\item $\frac{d}{dt} \left( 12 - 9\sin\left( \frac{\pi}{12}t \right) \right)$

\item $\frac{d}{d\theta} \left( \sec\theta \right)$

\item $\frac{d}{d\theta} \left( \csc\theta \right)$

\item $\frac{d}{d\theta}\left( \cos^2\theta \right)$

\item $\frac{d}{dt} \left( \sqrt{4 + \sin^4 t} \right)$  

\item  $\frac{d}{d\theta} \left( \sec^2\theta \right)$

\item $\frac{d}{dt} \left( \frac{1}{5+3\cos (t/2)}   \right)$
...
\begin{exercise}  \label{E:dfdr3hgret333}
