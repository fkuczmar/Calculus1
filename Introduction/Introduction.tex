...
So the first step is to find a function
\[
   s = f(h), h\geq 0 ,
\]
that expresses the distance to the horizon (still measured in miles) in terms of our height above the earth's surface, \emph{now measured in miles instead of feet}. We'll suppose the earth to be a perfect sphere of radius $4000$ miles. The distance to the horizon is the arclength $AT$ below, meaured along the surface of the earth (you can think of this distance as the radius of the spherical disk that is the portion of the earth we can see). Our height is the distance $AP$. 
...
\begin{hint}
Use right triangle $\Delta OTP$ to find an expression for the radian measure of angle $\angle POT$. Then use this angle to find an expression for the arclength $A$.
\end{hint}

(b) Now suppose we are $240$ miles above the surface of the earth. We wish to approximate how a small change in our altitude changes the distance to the horizon.
