...
\title{Introduction, Part 2}

\newcommand{\pskip}{\vskip 0.1 in}

\begin{document}
\begin{abstract}
Introduction to small changes and scaling factors.
\end{abstract}
\maketitle
...
(b) The data in the table above suggests an approximate proportional relationship between $\Delta A$ and $\Delta s$. We can guess the \emph{scaling factor} from the fifth column. As $s\to 5$ (as $s$ approaches $5$), it looks like the ratio $\Delta A/\Delta s$ approaches some number, what we call the \emph{constant of proportionality} or the \emph{scaling factor}.
...
(c) The scaling factor is called the derivative, in this case of the function $A=s^2$, at the input $s=5$. We write this as
\[
   \frac{dA}{ds}\Big|_{s=5} = \answer{10} .
\]

(d) We could have taken an algebraic approach to determe the scaling instead. The idea is to first simplfiy the quotient $\Delta A/\Delta s$ as
\begin{align*}
...
The advantage of this algebraic approach is that we can now compute the scaling factor as a limit:
\begin{align*}
...
(ii) Write an equation for the line through the point $P$ with slope equal to the scaling factor in the line below and on Line 8 in the desmos worksheet:
\[
...
\]
square feet. The scaling factor $10$ has units $ft^2/ft = ft$.
}

\emph{This means that if we increase the side length of a square from $5$ feet to $5.1$ feet, for example, the area of the square would increase by approximately}
\[
  \Delta A = \left( 10 \text{ ft} \right)(0.1 \text{ ft}) = 1 \text{ ft}^2.
\]

\item{Zoom in close enough to the graph of the function $A=f(s)=s^2$ near the point $P(5,25)$ and the graph looks like a line with slope equal to the scaling factor.}

\item{We can compute the scaling factor as the limit
\[
...
(f) Explain the meaning of the scaling factor in parts (d) and (e). Be sure to include units in your explanation.
