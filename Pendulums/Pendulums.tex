...
\begin{example} \label{ExKdfMMefr3989}
A cycloid is the curve traced by a point attached to a circle as the circle rolls on a line.

Drag the slider $\theta_2$ in Line 2 of the worksheet below to roll the circle.

On which line does the circle roll? Explain how you know.
\begin{freeResponse}
\end{freeResponse}

\begin{onlineOnly}
    \begin{center}
\desmos{rs6esnvlfi}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/calculator/rs6esnvlfi}{152: Cycloidal Pendulum 2}

We'll parameterize the cycloid above traced by the point $P$ in terms of the marked rotation angle $\theta$ (called $\theta_2$ in the worksheet above) of the rolling circle.  with radius $a$ meters. Do this as follows. 

\begin{enumerate}
\item Express the components of the vector $\overrightarrow{OC}$ from the origin to the center of the circle in terms of $\theta$ and the radius $a$ of the circle.
\[
     \overrightarrow{OC} = \langle \answer{a\theta}  , \answer{a} \rangle .
\]

\item Express the components of the vector $\overrightarrow{CP}$ in terms of $\theta$ and $a$.
\[
     \overrightarrow{CP} = \langle \answer{a\sin \theta}  , \answer{-a\cos\theta} \rangle .
\]

\item Use parts (a) and (b) to express the components of the vector $\overrightarrow{OC}$ in terms of $\theta$ and $a$.
\begin{align*}
       \overrightarrow{OP} &= \overrightarrow{OC} + \overrightarrow{CP} \\
                                     &= \langle  \answer{a\theta + a\sin\theta} , \answer{a - a\cos\theta} \rangle .
\end{align*}
...
We'll measure the ball's gravitational potential energy $mgh$ upward from the $y$-axis, so that $h=y$. 

We'll assume the the ball is released from the top of the cycloid at parameter $\theta=\pi$ and first express the speed of the ball in terms of its height $y$. In this case the mechanical energy of the ball (in terms of $m$, $g$, and $a$) is always
\[
  E = \answer{2mga} .
\]
...
\end{align*}
for the mass to travel the differential distances $ds$ meters along the cycloid at speeds of $v = \sqrt{g(4a - y )}$ m/sec.
...
So it remains only to express express the differential arclength $s$ in terms of the parameter $\theta$. Here's the computation.
\begin{align*}
   ds & = \sqrt{(dx)^2 + (dy)^2}    \\
...
\begin{align*}
   dt & = \frac{ds}{v} \\
       & = \frac{\answer{2a\cos(\theta/2)}\, d\theta}{\sqrt{2ga(\answer{1+\cos\theta})}} .
\end{align*}

And since $-\pi \leq \theta \leq \pi$, we know $\sqrt{\cos^2(\theta/2)} = \cos(\theta/2)$ and using the double-angle formula for $\cos\theta$ gives the surprising result that
\[
