...
\begin{enumerate}
\item To see if these average stretching factors appear to approach some value as $v\to 2$ (ie. as $v$ approaches $2$) we'll use the Table in Line 3 of the worksheet above. But first you'll need to input the correct expression for the average stretching function in Line 2. Then use the table to approximate the stretching factor at the input $L=2$.

\item Repeat part (a) to approximate the stretching factor at the input $L=9.5$.

\item Repeat part (a) to approximate the stretching factor at the right end where $L=10$.
\end{enumerate}
...
\item Finally, approximate the local stretching factor at $L=\pi$ numerically by finding entering an expression for the average stretching factor $k(v)$ between inputs $L=2$ and $L=v$ in Line 3 of the worksheet above.
