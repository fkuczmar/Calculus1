...
The \emph{global stretching factor} for a linear function like %The average rate of change of the function,
\[
      H = g(L) = 2L \, , \, 0\leq L \leq 4 ,
\]
...
\item Zoom in close enough to the stretched band near the highlighted output (in black, for the input $L=2$) to make the stretching factor look constant. Then approximate this factor
...
For example, at the input $L=2$, we'll to first compute the \emph{average stretching factors} 
\begin{align*}
...
\end{align*}
over the interval $v\leq L \leq 2$ (when $v<L$) or the interval $2 \leq L \leq v$ (when $v>2$). Then we'll check to see if these average stretching factors appear to approach some value as $v\to 2$ (ie. as $v$ approaches $2$).
