...
We can describe linear growth by saying a population grows at a constant rate of $0.5$ million bacteria/hour. Or we could  avoid the idea of an instantaneous growth rate by saying that the population increases at the average rate of $0.5$ million bacteria/hour over \emph{every} time interval.
...
\[
 f(t-3) =  2^{t-3} = (2^t)(2^{-3}) = \frac{1}{8} f(t) , 
\]
\[
      8 f(t-3) = f(t).
\]
...
The same kind of reasoning shows that for any for any exponential function, the growth rate is proportional to the population. Here's a more computational proof of this fact for the function $f(t)=P_0 2^t$.


Over the time interval from time $t$ hours past noon to time $t+\Delta t$ hours past noon the population increases by 
\begin{align*}
  \Delta P &=  f(t + \Delta t) - f(t)  \\ 
                               ...
                               &= P (2^{\Delta t} - 1) .
\end{align*}
millions of bacteria. Here $P=f(t)$ is the population at the start of the time interval.

So over the period from $t$ hours past noon to $t+\Delta t$ hours past noon the average growth rate is 
\[
    \frac{\Delta P}{\Delta t} = P \left( \frac{2^{\Delta t} - 1}{\Delta t}  \right) ,
\]
measured in millions of bacteria/hour.

Then at time $t$ hours past noon, the instantaneous growth rate (in millions of bacteria/hour) is
\begin{align*}
 \frac{dP}{dt} &= \lim_{\Delta t\to 0} \frac{\Delta P}{\Delta t} \\
                     &=  \lim_{\Delta t\to 0} P \left( \frac{2^{\Delta t} - 1}{\Delta t}  \right) \\
                     &= P \lim_{\Delta t\to 0}\frac{2^{\Delta t} - 1}{\Delta t} .
\end{align*}

 This shows what we wanted, that at any instant the growth rate is proportional to the poluation. Expressed differently, this says that the relative growth rate (measured in $\text{hr}^{-1}$) is constant and equal to
\[
        \frac{1}{P} \cdot \frac{dP}{dt} = \lim_{\Delta t\to 0}\frac{2^{\Delta t} - 1}{\Delta t} .
\]

The best we can do here is to approximate the above limit numerically, to get the relative growth rate as
\[
  \frac{1}{P} \cdot \frac{dP}{dt} \sim 0.6931 \text{ hr}^{-1} \sim 69.31\%/\text{hr}.
\]

{\bf Key Point:} Over a one-hour period, the population doubles. So over a one-hour period the population increases at a relative average rate of $100\%$/hour. This is greater than the relative instantaneous growth rate of $69.31\%$/hr because of compounding.
...
\begin{question} \label{Qpvem33}
At noon a cup of coffee has a temperature of $80^\circ$C. The coffe is in a room held at a constant temperature of $20^\circ$C. Five minutes later the temperature of the coffee is $70^\circ$C.

Suppose that the difference in temperature between the coffee and the room decreases exponentially.

\begin{enumerate}
\item Describe precisely how the temperature difference changes.

\item Use your description from part (a) to find a function
\[
     D = f(m) \, , \, m \geq 0,
\]
that expresses the temperature difference (in Celsius degrees) in terms of the number of minutes past noon. Do \emph{not} use $e$.

\item Use part (b) to find a function
\[
  T = g(m) \, , \, m\geq 0,
\]
that expresses the temperature of the coffee (in Celsius degrees) in terms of the number of minutes past noon. Do \emph{not} use $e$.

\item Would you expect the temperate of the coffee to be decreasing at a rate greater than or less than $2^\circ$C/min at noon? Explain your reasoning.

\item Find the rate at which the coffee is cooling at noon.

\item Find a function
\[
  r = h(T) 
\]
that expresses the rate (in $^\circ$C/min) at which the temperature of the coffee is changing in terms of its temperature.

\item Find the rate at which the temperature of the coffee is changing when its temperature is $50^\circ$C.
