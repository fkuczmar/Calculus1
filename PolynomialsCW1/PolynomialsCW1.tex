...
\[
   y = f(x)=ax^6, 
\]
where $a$ is a constant and the variables $x$, $y$ are measured in meters.
...
\[
     \frac{dy}{dx}\Big|_{x=a} =   \frac{d}{dx}\left( x^6 \right)\Big|_{x=a} .
\]

\item Check that your expression for the derivative has the correct units.
...
\item Interpret the above derivative as a scaling factor using the language of small changes.
