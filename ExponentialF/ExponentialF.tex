...
\begin{exercise} \label{EdDFERT}
Find an expression for the derivative
\[
  \frac{d}{dt}\left(  e^{\cos t} \right) .
\] 

\begin{explanation}
We need to use the chain rule because we are differentiating the composition of functions. So let
\[
    y = e^{\cos t}
\]
and 
...
\begin{align*}
   \frac{dy}{dt}  &= \frac{dy}{du }\cdot \frac{du}{dt}  \\
                        &= \left( \frac{d}{du}\left( e^u \right) \right) \left(  \frac{d}{dt} (\cos t) \right)  \\
                        &= e^u \, (-\sin t)  \\
                         &= (-\sin t)e^{\cos t} .
 \end{align*}
\end{explanation}
...
\begin{exercise}  \label{EKDFLedsadf}
Find expressions for each of the following derivatives. Follow the previous exercise exactly in using the chain rule.

\begin{enumerate}
\item $\frac{d}{dt} \left( 1000 e^{\frac{t}{5}}  \right)$

\item $\frac{d}{dt}\left(  \frac{10}{3+ 5e^t}    \right)$

\item $\frac{d}{dx} \left(  \sin (e^{4x})  \right)$

\item $\frac{d}{dt} \left( 1000 (2^t)  \right)$  

\emph{Hint:} Write $2^t$ as
\[
 2^t = \left( e^{\ln 2} \right)^t = e^{t \ln 2}
\]
