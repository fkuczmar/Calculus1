...
For example, the relative change in the population over a one hour period is
\[ 
        2^1 - 1 = 1 = 100\%,
\]
and over a half-hour period the relative change is
\[
...
We still have not answered the question about how to describe exponential growth \emph{without} a formula. What we know so far is that the relative average rate of change of an exponential function is constant over a time interval of fixed duration.  And Exploration 4 might have suggested that the relative instantaneous growth rate 
\[
...
\]
Then the relative instantaneous growth rate of the function $f(t)=3(2^t)$ is 
\begin{align*}
...
So our population increases at the constant relative (instantaneous) rate of approximately $69.3\%$/hr. The exact relative rate turns out to be
\[
...
\item Find the one-hour growth factor.

\item Use part (a) to find a function that expresses the population (in millions of bacteria) in terms of the number of hours past noon. Do not use $e$.
...
\begin{enumerate}
\item Use common sense to find its relative instantaneous growth rate. Then use calculus to verify your answer with a computation.
\end{enumerate}
\end{enumerate}
