...
\[
    \log_2 100.
\] 
Approximate the value of this expression \emph{without} a calculator.
\begin{freeResponse}
\end{freeResponse}
...
\[
     \frac{dP}{dt} \cdot \frac{dt}{dP} = \answer{1}
\]
and
\[
   \frac{dt}{dP} = \frac{1}{\frac{{\answer {dP}}}{\answer{dt}}}.
\]


\item Use part (c) to express the derivative
\[
 \frac{d}{dP} \left(  \ln P \right ) \, , \, P>0.
\]
in terms of $P$.
\begin{freeResponse}
\end{freeResponse}
...
Find each derivative twice.

\begin{enumerate}
\item By using the chain rule \emph{without} first simplifying each function.

\item By first simplifying each function and then differentiating.
\end{enumerate}
...
\item Explain why it would be incorrect to say
\[
...
\item A reflection about the origin takes the graph of a function $y=f(x)$ to the graph of a function $y=g(x)$. Describe the transformation that takes the graph of $y=f^\prime(x)$ to the graph of $y=g^\prime(x)$. Explain your reasoning.
...
\[
    y = f(x) = 3\ln \left| \frac{x^2+1}{x^3-1} \right|
\]
at the point on the curve with $x$-coordinate $x=-1$.

Do \emph{not} use the quotient rule. Instead start by simplifying the expression for $f(x)$.
\end{question}


\begin{question} \label{Qpf3r033dvf}
Then find simplified expressions for each of the following derivatives. Find also the domain of each function being differentiated.

\begin{enumerate}
\item $\frac{d}{dx} \left( \ln\left(x+\sqrt{x^{2}+1}\right) \right)$

\item $\frac{d}{dx} \left( \ln\left(x+\sqrt{x^{2}-1}\right) \right)$

\item $\frac{d}{dx} \left( \sqrt{\frac{1-x}{1+x}} \right)$
\end{enumerate}
...
Find the $y$-coordinates of all points on the curve
\[
    y = 3\ln \left( 1 + \cos\theta \right)
\]
