...
\item Sketch an accurate graph of the function $v = f(x)$. Use graph paper and
 choose a large scale so that the curve is big. Be sure to label the axes with the
 appropriate variable names and units.
 ii) Use your graph to make a reasonably accurate graph of the function s =
 g(t). Use graph paper and choose a large scale, being sure to match the horizontal
 scale from part i). Explain your method.
 iii) Use the geometry of the curve v = f(x) to nd an explicit expression
 (without the integral) for the function s = g(t).
 iv) Di erentiate the function s = g(t) to con rm your expression is correct
