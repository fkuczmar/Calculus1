...
Due to a printing error, the graph of the function $y=f(x) = 234x^4$ is missing below. All we see is a point $A$ on the graph of the function. 

\begin{onlineOnly}
    \begin{center}
\desmos{pjyqjtdcxmj}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/calculator/pjyqjtdcxm}{151: Printers Error 2}

\emph{Without} sketching a graph, drag the slider $m$ to draw the tangent line to the curve $y=234x^4$ below. Explain your reasoning.
...
\begin{onlineOnly}
    \begin{center}
\desmos{szrzof5ikv}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/calculator/szrzof5ikv}{151: Stock Price}

\begin{enumerate}
\item Is the price increasing or decreasing at 10am? Use the graph to estimate this rate. Then use the function to compute the exact rate. Include units.

\item Use the sliders $m$ and $b$ to control the line $P=b+mt$ and approximate when the price is decreasing at the rate of $(\$1.8/\text{share})/\text{hr}$. Then use algebra to compute the exact time(s).

\item Use the graph to approximate the minimum and maximum prices of the stock during the eight-hour period. Then use algebra to compute these exact prices.

\item Use the slider $u$ (another name for the variable $t$) to approximate the time(s) when the price is decreasing at the fastest rate. Then use algebra to compute the exact time(s).



\item Use the graph to approximate the end of the time interval beginning at 9am over which the price decreases at the greatest average rate.  The use algebra to find the exact time.
%\item Use technology (desmos or wolfram alpha) to approximate this time.
 

\item At what relative rate is the price changing at 9am?

\item Use algebra and technology to find the time(s) when the price is decreasing at the relative rate of $10\%$/hour.
