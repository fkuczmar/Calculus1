...
\item Use the graph to approximate the minimum and maximum prices of the stock during the eight-hour period. Then use calculus and algebra to compute these exact prices. Click the arrow below for a hint.

\begin{expandable}
What is the value of the derivative $dP/dt$ when the price is a maximum/minimum?
\end{expandable}

\item Use the slider $u$ (another name for the variable $t$) to approximate the time(s) when the price is decreasing at the fastest rate. Then use calculus to compute the exact time(s). Click the arrow below for a hint.

\begin{expandable}
It helps to graph the fuction
\[
...
\item The use calculus and algebra to find the end of the time interval beginning at 9am over which the price decreases at the greatest average rate.  Click the arrow below for a hint.

\begin{expandable}
Think abou the function
\[
